# 环境要求与依赖说明

本文档说明KamaitachiGo项目的运行环境、依赖管理和构建方法。

---

## 📋 环境要求

### 1. 操作系统
- Windows 10/11 (已测试) ✅
- Linux (理论支持)
- macOS (理论支持)

### 2. Go语言环境
- **Go版本**: 1.21 或更高
- **推荐版本**: Go 1.24+
- **Toolchain**: go1.24.5

#### 检查Go版本
```powershell
go version
```

#### 下载Go
如果未安装Go或版本过低，请访问：https://golang.org/dl/

### 3. 外部服务

#### etcd (必需)
- **用途**: 服务注册与发现
- **版本**: 3.5+
- **端口**: 2379 (客户端), 2380 (对等通信)

**下载地址**: https://github.com/etcd-io/etcd/releases

**验证etcd运行**:
```powershell
curl http://localhost:2379/version
```

详细安装步骤请参见下方 [外部服务安装指南](#外部服务安装指南)

---

## 📦 项目依赖

### 核心依赖 (直接依赖)

项目使用Go Modules管理依赖，所有依赖定义在`go.mod`文件中。

| 包名 | 版本 | 用途 |
|------|------|------|
| `github.com/gin-gonic/gin` | v1.9.1 | Web框架 - HTTP路由和中间件 |
| `github.com/go-ini/ini` | v1.67.0 | 配置文件解析 |
| `github.com/sirupsen/logrus` | v1.9.3 | 日志库 |
| `go.etcd.io/etcd/client/v3` | v3.5.10 | etcd客户端 - 服务发现 |
| `golang.org/x/time` | v0.5.0 | 限流器 - Token Bucket |
| `modernc.org/sqlite` | v1.39.1 | SQLite数据库 (纯Go实现) |

### 间接依赖 (自动管理)

Go Modules会自动管理所有间接依赖，详见`go.sum`文件。

---

## 🌐 外部服务安装指南

### etcd 安装与配置（必需）

etcd是分布式键值存储系统，用于服务注册与发现。

#### Windows 安装步骤

**1. 下载etcd**

访问官方GitHub发布页面：https://github.com/etcd-io/etcd/releases

下载Windows版本，例如：`etcd-v3.5.10-windows-amd64.zip`

**2. 解压安装**

```powershell
# 解压到指定目录（例如 C:\etcd）
Expand-Archive -Path etcd-v3.5.10-windows-amd64.zip -DestinationPath C:\etcd

# 进入目录
cd C:\etcd\etcd-v3.5.10-windows-amd64
```

**3. 启动etcd**

```powershell
# 方法1：直接启动（默认配置）
.\etcd.exe

# 方法2：指定数据目录启动
.\etcd.exe --data-dir=C:\etcd\data

# 方法3：后台启动（推荐）
Start-Process -FilePath ".\etcd.exe" -ArgumentList "--data-dir=C:\etcd\data" -WindowStyle Hidden
```

**4. 验证安装**

```powershell
# 检查版本
curl http://localhost:2379/version

# 检查健康状态
curl http://localhost:2379/health

# 预期输出：{"health":"true"}
```

**5. 设置开机自启（可选）**

创建Windows服务或添加到启动项：

```powershell
# 创建启动脚本：C:\etcd\start_etcd.bat
@echo off
cd C:\etcd\etcd-v3.5.10-windows-amd64
start /B etcd.exe --data-dir=C:\etcd\data

# 将此批处理文件添加到启动文件夹
# Win+R 输入 shell:startup 打开启动文件夹，放入快捷方式
```

#### Linux 安装步骤

```bash
# 1. 下载
wget https://github.com/etcd-io/etcd/releases/download/v3.5.10/etcd-v3.5.10-linux-amd64.tar.gz

# 2. 解压
tar -xzf etcd-v3.5.10-linux-amd64.tar.gz

# 3. 移动到系统路径
sudo mv etcd-v3.5.10-linux-amd64/etcd* /usr/local/bin/

# 4. 启动
etcd --data-dir=/var/lib/etcd

# 5. 验证
curl http://localhost:2379/version
```

#### etcd 配置说明

**默认端口**：
- `2379`: 客户端通信端口（项目使用此端口）
- `2380`: 对等通信端口（集群内部使用）

**常用配置参数**：

```powershell
# 指定监听地址
.\etcd.exe --listen-client-urls=http://0.0.0.0:2379 `
           --advertise-client-urls=http://localhost:2379

# 指定数据目录
.\etcd.exe --data-dir=D:\etcd_data

# 启用日志
.\etcd.exe --log-outputs=stdout --log-level=info
```

**项目配置文件中的etcd地址**：

在 `conf/master.ini` 和 `conf/slave*.ini` 中：
```ini
[etcd]
endpoints = 127.0.0.1:2379
dial_timeout = 5
```

#### etcd 管理命令

```powershell
# 查看所有键
etcdctl get --prefix ""

# 查看服务注册信息（项目运行后）
etcdctl get --prefix "/services/"

# 删除所有服务信息（清理用）
etcdctl del --prefix "/services/"

# 查看etcd成员
etcdctl member list
```

#### etcd 故障排查

**问题1：端口被占用**

```powershell
# 查看端口占用
netstat -ano | findstr "2379"

# 杀死占用进程
taskkill /PID <进程ID> /F
```

**问题2：数据损坏**

```powershell
# 停止etcd
Get-Process etcd | Stop-Process -Force

# 删除数据目录
Remove-Item -Recurse -Force C:\etcd\data

# 重新启动
.\etcd.exe --data-dir=C:\etcd\data
```

**问题3：无法连接**

```powershell
# 检查防火墙
# Windows防火墙 -> 高级设置 -> 入站规则 -> 新建规则
# 允许端口 2379 和 2380

# 或临时关闭防火墙测试
Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled False
```

---

### SQLite 说明（无需安装）

本项目使用 `modernc.org/sqlite`，这是SQLite的**纯Go实现**。

**优势**：
- ✅ 无需安装SQLite
- ✅ 无需CGO编译环境
- ✅ 编译后的程序完全独立
- ✅ 跨平台兼容

**数据文件**：
- 位置：`data/master.db`, `data/slave1.db`, `data/slave2.db`
- 格式：SQLite3数据库文件
- 自动创建：首次运行时自动创建

**查看数据库内容（可选工具）**：

1. **VS Code DuckDB扩展**（你已有）
   - 可以打开SQLite文件查看

2. **SQLite Browser**
   - 下载：https://sqlitebrowser.org/
   - 免费、开源、跨平台

3. **命令行工具**（如果需要）
   ```powershell
   # 使用项目自带的检查工具
   go run .\tools\check_db.go
   ```

**数据导入**：

```powershell
# 从SQL文件导入数据
go run .\cmd\import\import_sql.go `
    -sql="f10sql\finance_quarter_stock_benefit.sql" `
    -db="data\master.db"
```

---

### 可选服务和工具

#### 1. PowerShell 7+（推荐升级）

项目测试脚本使用PowerShell，推荐使用PowerShell 7：

```powershell
# 下载地址
https://github.com/PowerShell/PowerShell/releases

# 安装后验证
pwsh --version
```

#### 2. Git（版本控制）

```powershell
# 下载地址
https://git-scm.com/downloads

# 验证
git --version
```

#### 3. Postman / Insomnia（API测试）

用于手动测试API接口：
- Postman: https://www.postman.com/downloads/
- Insomnia: https://insomnia.rest/download

#### 4. 性能监控工具

**Windows性能监视器**（已内置）：
```powershell
# 打开任务管理器
taskmgr

# 打开资源监视器
resmon
```

**进程监控**：
```powershell
# 查看节点进程
Get-Process | Where-Object {$_.Name -match "master|slave"}

# 监控资源使用
Get-Process master,slave | Select-Object Name,CPU,WS,PM
```

---

### 快速验证所有环境

运行以下脚本验证所有环境是否就绪：

```powershell
Write-Host "========== Environment Check ==========" -ForegroundColor Cyan

# 1. 检查Go
Write-Host "`n[1] Go Version:" -ForegroundColor Yellow
go version

# 2. 检查etcd
Write-Host "`n[2] etcd Status:" -ForegroundColor Yellow
try {
    $response = curl http://localhost:2379/version 2>&1
    Write-Host "etcd is running!" -ForegroundColor Green
    $response
} catch {
    Write-Host "etcd is NOT running!" -ForegroundColor Red
    Write-Host "Please start etcd first." -ForegroundColor Red
}

# 3. 检查项目依赖
Write-Host "`n[3] Go Dependencies:" -ForegroundColor Yellow
cd D:\data\1028\KamaitachiGo
go mod verify

# 4. 检查编译文件
Write-Host "`n[4] Compiled Binaries:" -ForegroundColor Yellow
if (Test-Path "bin\master.exe") {
    Write-Host "master.exe: OK" -ForegroundColor Green
} else {
    Write-Host "master.exe: NOT FOUND" -ForegroundColor Red
}

if (Test-Path "bin\slave.exe") {
    Write-Host "slave.exe: OK" -ForegroundColor Green
} else {
    Write-Host "slave.exe: NOT FOUND" -ForegroundColor Red
}

# 5. 检查数据库文件
Write-Host "`n[5] Database Files:" -ForegroundColor Yellow
Get-ChildItem data\*.db | ForEach-Object {
    Write-Host "$($_.Name): $([math]::Round($_.Length/1MB, 2)) MB" -ForegroundColor Green
}

Write-Host "`n========== Check Complete ==========" -ForegroundColor Cyan
```

将上述脚本保存为 `check_environment.ps1`，运行即可一键检查所有环境。

---

## 🔧 依赖安装

### 方法1：自动安装（推荐）

```powershell
# 进入项目目录
cd D:\data\1028\KamaitachiGo

# 下载所有依赖（自动根据go.mod）
go mod download

# 或者直接构建（会自动下载依赖）
go build .\cmd\master\main.go
```

### 方法2：手动验证依赖

```powershell
# 验证依赖完整性
go mod verify

# 清理未使用的依赖
go mod tidy

# 查看依赖树
go mod graph
```

### 配置Go代理（可选，加速下载）

如果下载依赖速度慢，可以配置国内代理：

```powershell
# 设置Go代理
go env -w GOPROXY=https://goproxy.cn,direct

# 或使用阿里云代理
go env -w GOPROXY=https://mirrors.aliyun.com/goproxy/,direct
```

---

## 🏗️ 完整构建流程

### 从零开始构建项目

```powershell
# 1. 克隆或下载项目
cd D:\data\1028\KamaitachiGo

# 2. 验证Go环境
go version

# 3. 下载依赖
go mod download

# 4. 编译所有组件
# 编译Master节点
go build -o bin\master.exe .\cmd\master\main.go

# 编译Slave节点
go build -o bin\slave.exe .\cmd\slave\main.go

# 编译单节点Server
go build -o bin\server.exe .\cmd\server\main.go

# 单独构建压测工具
go build -o bin/benchmark_scenarios.exe tools/benchmark_scenarios.go

# 编译数据库检查工具
go build -o bin\check_db.exe .\tools\check_db.go

# 5. 验证编译结果
dir bin\*.exe
```

---

## 📁 项目结构说明

```
KamaitachiGo/
├── go.mod                  # Go模块定义文件（类似requirements.txt）
├── go.sum                  # 依赖校验和文件（自动生成）
├── bin/                    # 编译后的可执行文件
├── cmd/                    # 主程序入口
│   ├── master/            # Master节点
│   ├── slave/             # Slave节点
│   ├── server/            # 单节点服务
│   └── import/            # SQL导入工具
├── internal/               # 内部代码（不对外暴露）
│   ├── cache/             # LRU缓存实现
│   ├── handler/           # HTTP处理器
│   ├── middleware/        # 中间件（限流、熔断）
│   ├── model/             # 数据模型
│   ├── repository/        # 数据访问层
│   └── service/           # 业务逻辑层
├── pkg/                    # 公共库（可对外暴露）
│   └── json/              # JSON工具（jsoniter封装）
├── conf/                   # 配置文件
├── data/                   # 数据库文件
├── logs/                   # 日志文件
└── tools/                  # 工具程序
```

---

## 🚀 快速开始

### 1. 确认环境

```powershell
# 检查Go版本
go version

# 检查etcd
curl http://localhost:2379/version

# 验证依赖
go mod verify
```

### 2. 构建项目

```powershell
# 编译所有必要组件
go build -o bin\master.exe .\cmd\master\main.go
go build -o bin\slave.exe .\cmd\slave\main.go
go build -o bin\benchmark_scenarios.exe .\tools\benchmark_scenarios.go
```

### 3. 启动集群

```powershell
# 使用脚本启动3节点集群
.\scripts\start_cluster.ps1

# 等待启动完成
Start-Sleep -Seconds 10
```

### 4. 验证运行

```powershell
# 测试健康检查
curl http://localhost:8080/health
curl http://localhost:8081/health
curl http://localhost:8082/health
```

### 5. 运行测试

```powershell
# 运行压测
.\bin\benchmark_scenarios.exe -scenario 1 -requests 5000 -concurrent 50 -target "http://localhost:9000/data"
```

---

## 🔍 依赖说明详解

### 1. Web框架 - Gin

```go
github.com/gin-gonic/gin v1.9.1
```

**用途**: 
- HTTP路由
- 中间件支持
- 请求/响应处理

**为什么选择Gin**:
- 性能优秀（基于httprouter）
- 轻量级
- 中间件生态丰富

### 2. 数据库 - SQLite (纯Go)

```go
modernc.org/sqlite v1.39.1
```

**用途**: 
- 存储财务数据
- 每个节点独立数据库

**为什么选择modernc.org/sqlite**:
- 纯Go实现，无需CGO
- 跨平台，易部署
- 无需额外数据库服务

### 3. 服务发现 - etcd Client

```go
go.etcd.io/etcd/client/v3 v3.5.10
```

**用途**: 
- 节点注册
- 服务发现
- 分布式协调

### 4. 限流 - Token Bucket

```go
golang.org/x/time v0.5.0
```

**用途**: 
- 实现Token Bucket限流算法
- QPS控制

---

## 🛠️ 开发工具

### 推荐的开发工具

1. **IDE**: 
   - VS Code + Go扩展 (推荐)
   - GoLand

2. **调试工具**:
   - Delve (Go调试器)
   ```powershell
   go install github.com/go-delve/delve/cmd/dlv@latest
   ```

3. **代码格式化**:
   ```powershell
   # 格式化代码
   go fmt ./...
   
   # 代码检查
   go vet ./...
   ```

4. **性能分析**:
   ```powershell
   # 生成性能分析
   go test -cpuprofile=cpu.prof -memprofile=mem.prof -bench .
   ```

---

## 📌 依赖更新

### 更新所有依赖到最新版本

```powershell
# 查看可更新的依赖
go list -u -m all

# 更新指定依赖
go get -u github.com/gin-gonic/gin

# 更新所有依赖（谨慎使用）
go get -u ./...

# 更新后清理
go mod tidy
```

### 固定依赖版本（生产环境推荐）

当前`go.mod`中的版本都是经过测试的稳定版本，**不建议随意更新**。

---

## 🐛 常见问题

### Q1: 依赖下载失败

**问题**: `go: github.com/xxx: connection refused`

**解决方案**:
```powershell
# 方案1：配置代理
go env -w GOPROXY=https://goproxy.cn,direct

# 方案2：使用VPN

# 方案3：手动下载依赖包
go mod download
```

### Q2: 编译错误 - 找不到包

**问题**: `package xxx is not in GOROOT`

**解决方案**:
```powershell
# 1. 清理模块缓存
go clean -modcache

# 2. 重新下载依赖
go mod download

# 3. 验证go.mod
go mod verify

# 4. 重新编译
go build .\cmd\master\main.go
```

### Q3: CGO相关错误

**问题**: `cgo: exec gcc: not found`

**说明**: 
本项目特意选择了纯Go依赖（如`modernc.org/sqlite`），**不需要CGO**。

如果遇到CGO错误，可以显式禁用：
```powershell
$env:CGO_ENABLED=0
go build .\cmd\master\main.go
```

### Q4: 版本不兼容

**问题**: `go.mod requires go >= 1.24`

**解决方案**:
```powershell
# 方案1：升级Go版本（推荐）
# 下载最新版本：https://golang.org/dl/

# 方案2：降低项目要求的Go版本（不推荐）
# 修改go.mod第一行：go 1.21
```

---

## 📊 依赖分析

### 查看依赖大小

```powershell
# 查看编译后的二进制大小
dir bin\*.exe | Select-Object Name, Length

# 查看依赖关系
go mod graph | findstr "github.com/gin-gonic/gin"
```

### 依赖可视化

```powershell
# 安装依赖可视化工具
go install golang.org/x/exp/cmd/modgraphviz@latest

# 生成依赖图
go mod graph | modgraphviz | dot -Tpng -o dependencies.png
```

---

## ✅ 检查清单

部署前检查：
- [ ] Go版本 >= 1.21
- [ ] etcd服务运行中
- [ ] 所有依赖下载完成 (`go mod download`)
- [ ] 依赖验证通过 (`go mod verify`)
- [ ] 所有组件编译成功
- [ ] 配置文件正确 (`conf/*.ini`)
- [ ] 数据库文件存在 (`data/*.db`)

---

## 📝 与Python的对比

| 特性 | Python | Go |
|------|--------|-----|
| 依赖文件 | requirements.txt | go.mod + go.sum |
| 包管理器 | pip | go get / go mod |
| 虚拟环境 | venv/virtualenv | 不需要（模块隔离） |
| 依赖安装 | `pip install -r requirements.txt` | `go mod download` |
| 锁定版本 | requirements.txt | go.sum |
| 构建产物 | .py文件（需Python） | .exe可执行文件（独立） |

**Go的优势**:
1. **单文件部署**: 编译后是独立可执行文件，无需安装Go环境
2. **自动依赖管理**: go.mod自动管理依赖关系
3. **依赖校验**: go.sum确保依赖完整性
4. **跨平台编译**: 可交叉编译到不同平台

---

## 🎯 总结

本项目使用Go Modules进行依赖管理，核心依赖包括：

1. **Gin** - Web框架
2. **SQLite** - 嵌入式数据库（纯Go）
3. **etcd client** - 服务发现
4. **jsoniter** - 高性能JSON
5. **golang.org/x/time** - 限流器

所有依赖都是纯Go实现，无需CGO，确保跨平台兼容性和易部署性。

---

**最后更新**: 2025-10-24  
**Go版本**: 1.24+  
**依赖包数量**: 7个直接依赖，60+个间接依赖

